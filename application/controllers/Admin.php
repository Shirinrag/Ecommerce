    ...
    function save_sub_category(){
        ...
        if(isset($array_entity) && !empty($array_entity)){
            ...
            if(isset($sub_category_data) && !empty($sub_category_data)){
                ...
                $category_id = $this->model->insertData('subcategory',array('category_id'=>$category_id,'sub_category_name'=>$sub_category_name,'sub_category_name_ar'=>$sub_category_name_ar,'sort_order'=>$sub_category_sort_order));
                if($category_id>0){
                    $data['status'] = '1';
    ...
    function submit_product(){
        ...
        if(!empty($_FILES['image_files']['name'][0])){
          ...
          if($this->upload->do_upload('file')){
            ...
            $product_array['image_name'] = 'uploads/'.$filename;  
        }
    }
    ...
    for($i=0;$i<$countfiles;$i++){
        ...
        if(!empty($_FILES['image_files']['name'][$i])){
          ...
          if($this->upload->do_upload('file')){
            $uploadData = $this->upload->data();
            $filename = $uploadData['file_name'];               
            $productarray['imagename'][] = 'uploads/'.$filename;

            $product_id=$this->model->insertData('product_gallery',$product_array);
        }
    }
...
function edit_product(){    
    ...
    //echo "<pre>";print_r($data['subcat_by_id']);die();
    $data['category_data'] = $this->model->getData('category',array('category_id'=>$data['product_data'][0]['category_id']));
