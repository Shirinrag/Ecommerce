...
class admin extends CI_Controller {
    ...
    function submit_product(){
        ...
        if(isset($_FILES['image_name']) && !empty($_FILES['image_name'])){
            $uploaddir = './products/';
            $filename = rand().basename($_FILES['image_name']['name']);
            $uploadfile = $uploaddir . $filename;
    ...
    function update_product(){
        ...
        if($_FILES['image_name']['name'] != ''){
            ...
            $image_name = $this->model->getValue('product','image_name',array('product_id'=>$_POST['product_id']));
        } 
                ...
                if(isset($session_id) && $session_id!=''){
                    ...
                    if(isset($product_data) && !empty($product_data)){
                        ...
                        if(isset($is_product_already) && empty($is_product_already)){
                            ...
                            $array_data = array(
                            'session_id'=>$session_id,'product_id'=>$product_id,'product_name'=>strtoupper($product_data[0]['product_name']),'price'=>$product_data[0]['price'],'qty'=>1,'unit_total'=>$product_data[0]['price']*1,'category_id'=>strtoupper($product_data[0]['category_id']),'category_name'=>strtoupper($category_data[0]['category_name'])
                            );
    ...
    function send_order_confirmation_mail($order_id,$deliver_address_id,$email_address){
        if($order_id!='' && $deliver_address_id!=''){
            ...
            if(isset($order_data) && !empty($order_data)){
                ...
                $subject = 'Your Marcos.in Order #'.$order_number;
                ...
                $headers = 'From: Marcos <myorder@Marcos.in>' . "\r\n" .
    ...
    function download_price_list(){
        ...
        fputcsv($output, array('Sr No', 'Product Id', 'Product Name','Category Id','Category Name','Sub Cat Id','Sub Category Name','Old Price','Sell Price','Unit Id','Unit Name','Stock Status','Stock value','Product Status'));

        // fetch the data
        $rows = $this->model->getDataOrderBy('product',array(),'product_name','ASC');
        ...
        if (isset($rows) && !empty($rows)) {
            foreach ($rows as $key => $row) {
                ...
                $prows = array( ++$key,$row['product_id'],strtoupper($row['product_name']),$cat_data[0]['category_id'],strtoupper($cat_data[0]['category_name']),$sub_cat_data[0]['sub_category_id'],strtoupper($sub_cat_data[0]['sub_category_name']),$row['old_price'],$row['price'],$unit_data[0]['unit_id'],$unit_data[0]['unit_name'],$stock_status_value,$stock_status,$product_status);
            ...
            if(isset($arr_data) && !empty($arr_data)){
                foreach ($arr_data as $key => $value) {
                    ...
                    $update_data = array(
                        ...
                        'category_id'=>(isset($value['D']) && $value['D']!='') ? $value['D'] : '',
                        'sub_category_id'=>(isset($value['F']) && $value['F']!='') ? $value['F'] : '',
                        'old_price'=>(isset($value['H']) && $value['H']!='') ? $value['H'] : '',
                        'price'=>(isset($value['I']) && $value['I']!='') ? $value['I'] : '',
                        'unit_id'=>(isset($value['J']) && $value['J']!='') ? $value['J'] : '',
                        'stock_status'=> (isset($value['L']) && $value['L']=='1') ? '1' : '0',
                    );
